...
static void collect_cb(string *k, int *v, void *userdata)
{
  CollectCtx *ctx = userdata;
  if (ctx->count < ctx->capacity)
  {
    ctx->keys[ctx->count] = (char *)*k;
    ctx->values[ctx->count] = *v;
  }
  ctx->count++;
}

TEST(map_foreach_empty)
{
  Map(string, int) m = Map_init(string, int);

  CollectCtx ctx;
  ctx.count = 0;
  ctx.keys = malloc(10 * sizeof(char *));
  ctx.values = malloc(10 * sizeof(int));

  Map_foreach(string, int, &m, collect_cb, &ctx);
...
TEST(map_foreach_single)
{
  ...
  ASSERT_TRUE(strcmp(ctx.keys[0], "A") == 0);
  ASSERT_TRUE(ctx.values[0] == 1);
...
TEST(map_foreach_multiple_sorted_order)
{
  Map(string, int) m = Map_init(string, int);
  Map_insert(string, int, &m, "C", 3);
  Map_insert(string, int, &m, "A", 1);
  ...
  ctx.values = malloc(ctx.capacity * sizeof(int));

  Map_foreach(string, int, &m, collect_cb, &ctx);

  ASSERT_TRUE(ctx.count == 3);

  // Should be in sorted order: A, B, C
  ASSERT_TRUE(strcmp(ctx.keys[0], "A") == 0);
  ASSERT_TRUE(ctx.values[0] == 1);

  ASSERT_TRUE(strcmp(ctx.keys[1], "B") == 0);
  ASSERT_TRUE(ctx.values[1] == 2);

  ASSERT_TRUE(strcmp(ctx.keys[2], "C") == 0);
  ASSERT_TRUE(ctx.values[2] == 3);
...
TEST(map_foreach_large)
{
  ...
  {
    keys[i] = malloc(16);
    snprintf(keys[i], 16, "K%02d", i);
    Map_insert(string, int, &m, keys[i], i);
  }
  ...
  ASSERT_TRUE(ctx.count == 20);

  // Verify sorted order: "K00", "K01", ...
  for (int i = 0; i < 20; i++)
  {
    char expected[16];
    snprintf(expected, 16, "K%02d", i);
    ASSERT_TRUE(strcmp(ctx.keys[i], expected) == 0);
    ASSERT_TRUE(ctx.values[i] == i);
  }
...
TEST_SUITE(
    ...
    // `For each` tests
    RUN_TEST(map_foreach_empty),
    RUN_TEST(map_foreach_single),
    RUN_TEST(map_foreach_multiple_sorted_order),
    RUN_TEST(map_foreach_large))