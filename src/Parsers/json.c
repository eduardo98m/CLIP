...
// --- Parse value ---
static JsonValue* parse_value(const char **s) {
    skip_ws(s);
    if (**s == '{') return parse_object(s);
    if (**s == '[') return parse_array(s);
    if (**s == '"') return parse_string(s);
    if (strncmp(*s, "true", 4) == 0) return parse_literal(s, "true", JSON_BOOL, 1);
    if (strncmp(*s, "false", 5) == 0) return parse_literal(s, "false", JSON_BOOL, 0);
    if (strncmp(*s, "null", 4) == 0) return parse_literal(s, "null", JSON_NULL, 0);
    return parse_number(s);
}

// --- Object ---
static JsonValue* parse_object(const char **s) {
    (*s)++; // skip '{'
...
// --- Array ---
static JsonValue* parse_array(const char **s) {
    (*s)++; // skip '['
...
// --- String ---
static JsonValue* parse_string(const char **s) {
    (*s)++; // skip opening "
    const char *start = *s;
    while (**s && **s != '"') {
...
// --- Number ---
static JsonValue* parse_number(const char **s) {
    const char *start = *s;
...
// --- Literal ---
static JsonValue* parse_literal(const char **s, const char *literal, JsonType type, int bool_val) {
    size_t len = strlen(literal);
    *s += len;
    JsonValue *v = new_value(type);
    if (type == JSON_BOOL) v->boolean = bool_val;
    return v;
}
    ...
    switch (v->type) {
        ...
        case JSON_ARRAY:
            for (size_t i = 0; i < v->array.size; i++) {
                JsonValue *elem = List_get(JsonValue_ptr, &v->array, i);
                Json_free(elem);
            }
